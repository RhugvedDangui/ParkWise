...
# ── Config ──────────────────────────────────────────────────────────────────
# Extract camera_id and video_path from CLI
camera_name = sys.argv[1] if len(sys.argv) > 1 else (input("Enter camera/location name (default: 'cam1'): ").strip() or "cam1")
video_path  = sys.argv[2] if len(sys.argv) > 2 else "media/carPark.mp4"
...
# Extract first frame from video to use as base background
cap = cv2.VideoCapture(video_path)
success, base_img = cap.read()
cap.release()

if not success or base_img is None:
...
def draw_legend(img):
    ...
    cv2.rectangle(overlay, (x0 - pad, y0 - pad),
                  (x0 + box_w, y0 + len(items) * h + pad), (30, 30, 30), -1)
...
while True:
    ...
    for slot_name, slot_data in parking_slots.items():
        x1, y1, x2, y2 = slot_data["x1"], slot_data["y1"], slot_data["x2"], slot_data["y2"]
        stype = slot_data.get("type", "normal")
        color = SLOT_COLORS.get(stype, SLOT_COLORS["normal"])
        label = SLOT_LABELS.get(stype, "P")
        ...
        cv2.putText(img, slot_name, (x1 + 3, y1 - 6),
    ...
    if selected_slot and selected_slot in parking_slots:
        ...
        cv2.putText(img, hint, (10, img.shape[0] - 30),
